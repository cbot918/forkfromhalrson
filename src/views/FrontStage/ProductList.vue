    ...
    <div class=" row">
      <nav class="col-3 bg-light float-start">
        <div class="position-sticky pt-3">
          <p>分類篩選</p>
          <select class="form-select" v-on:change="filtering($event.target.value)">
...
<script setup>
  ...
  const filterProducts = ref([]), categories = ref([])
  ...
  function filtering (category) {
    switch (category) {
      ...
      case 'favorite':
        ...
        filterProducts.value = products.value.filter(product => favorites.includes(product.id))
        break
      default:
