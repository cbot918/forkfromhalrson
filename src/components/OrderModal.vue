            ...
            <table class="table">
              <thead class="container">
                <tr class="row">
                  ...
                  <td class="col" v-text="date(order.create_at*1000)"></td>
                  <th class="col">付款日期</th>
                  <td class="col" v-text="order.paid_date?date(order.paid_date*1000):'等待付款中'"></td>
                </tr>
              </thead>
              <tbody class="container">
                <tr class="row">
                  <th class="col-6">商品名稱</th>
                  <th class="col text-end">單價</th>
                  <th class="col text-end">數量</th>
                ...
                <tr class="row" v-if="Object.values(order.products||{}).some(product=>product.coupon)">
                  <td class="col">折扣</td>
                  <td class="col text-end">{{currency(order.total/(1-Object.values(order.products)[0].coupon.percent))}}</td>
                  <td class="col">優惠價</td>
          ...
          <div class="modal-body">
            <div class="container">
              <p class="row">
                ...
                <span class="col" v-text="date(order.create_at*1000)"></span>
                <span class="col">付款日期</span>
                <span class="col" v-text="order.paid_date?date(order.paid_date*1000):'等待付款中'"></span>
              </p>
              <p class="row">
            ...
            <div class="container">
              ...
              <p class="row" v-if="Object.values(order.products||{}).some(product=>product.coupon)">
                ...
                <span class="col text-end">優惠價</span>
...
<script setup>
  ...
  const DOM = ref(null), form = ref(null), prop = defineProps(['id', 'type'])
  const orderStore = useOrderStore(), { editOrder, getOrder } = orderStore, { order, user } = storeToRefs(orderStore)
  let modal
  onMounted(function () {
